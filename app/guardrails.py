...
def check_query(query: str, document_text: str) -> tuple[bool, str]:
    ...
    We intentionally keep this lightweight — only blocking empty or
    nonsensical input. Off-topic detection is handled by the LLM's
    system prompt, which is more accurate and handles abstract phrasing
    like "summarise", "what stands out", "tell me about" — all of which
    score low on semantic similarity even when they're valid questions.
    """
    if len(query.strip()) < 3:
        return False, "Please enter a valid question."

    # Block input that is entirely numbers or special characters
